...
export function BudgetRow({
    ...
    const [newSub, setNewSub] = useState({ spec_no: "", vendor: "TBD", vendor_description: "", description: "", qty: "1 Ea.", unit_cost: "" })
    ...
    const handleChange = (field, value) => {
        setLocalItem((prev) => {
            ...
            if (field === "unit_cost" || field === "qty") {
                const unitCost = field === "unit_cost" ? parseFloat(value) || 0 : parseFloat(updated.unit_cost) || 0
                updated.extended = parseFloat((qtyNum * unitCost).toFixed(2))
            } else if (field === "extended") {
                const extVal = parseFloat(value) || 0
                updated.unit_cost = qtyNum > 0 ? parseFloat((extVal / qtyNum).toFixed(2)) : 0
            }
            return updated
        })
    }
    ...
    const handleAddSubItemSave = async () => {
        ...
        setNewSub({ spec_no: "", vendor: "TBD", vendor_description: "", description: "", qty: "1 Ea.", unit_cost: "" })
    ...
    return (
        <>
            ...
            <tr className={`border-b transition-colors hover:bg-muted/50 ${isEditing ? "bg-muted/30 ring-1 ring-inset ring-muted-foreground/20" : ""} ${isHidden ? "opacity-60" : ""}`}>
                ...
                <td className="p-2 align-middle font-medium w-[100px]">
                    <div className="flex items-center gap-1">
                        ...
                        {isEditing ? (
                            <Input value={localItem.spec_no || ""} onChange={(e) => handleChange("spec_no", e.target.value)} onKeyDown={handleKeyDown} className="h-8" />
                        ) : (
                            item.spec_no
                        )}
                    </div>
                ...
                <td className="p-2 align-middle w-[120px]">
                    {isEditing ? <Input value={localItem.vendor || ""} onChange={(e) => handleChange("vendor", e.target.value)} onKeyDown={handleKeyDown} className="h-8" /> : item.vendor}
                </td>

                {/* Vendor Desc */}
                <td className="p-2 align-middle hidden md:table-cell max-w-[150px] truncate">
                    {isEditing ? <Input value={localItem.vendor_description || ""} onChange={(e) => handleChange("vendor_description", e.target.value)} onKeyDown={handleKeyDown} className="h-8" /> : item.vendor_description}
                </td>

                {/* Description */}
                <td className="p-2 align-middle max-w-[200px] truncate">
                    {isEditing ? <Input value={localItem.description || ""} onChange={(e) => handleChange("description", e.target.value)} onKeyDown={handleKeyDown} className="h-8" /> : item.description}
                </td>

                {/* Room */}
                <td className="p-2 align-middle w-[120px]">
                    {isEditing ? <Input value={localItem.room_name || ""} onChange={(e) => handleChange("room_name", e.target.value)} onKeyDown={handleKeyDown} className="h-8" /> : item.room_name}
                </td>

                {/* Page No */}
                <td className="p-2 align-middle w-[60px] text-center">
                    {isEditing ? (
                        <Input type="number" value={localItem.page_no || ""} onChange={(e) => handleChange("page_no", parseInt(e.target.value) || "")} onKeyDown={handleKeyDown} className="h-8 text-center" />
                    ) : item.page_no}
                </td>

                {/* Qty */}
                <td className="p-2 align-middle w-[80px]">
                    {isEditing ? <Input value={localItem.qty || ""} onChange={(e) => handleChange("qty", e.target.value)} onKeyDown={handleKeyDown} className="h-8" /> : item.qty}
                </td>

                {/* Unit Cost */}
                <td className="p-2 align-middle w-[100px] text-right">
                    {isEditing ? (
                        <Input type="number" value={localItem.unit_cost || ""} onChange={(e) => handleChange("unit_cost", parseFloat(e.target.value))} onKeyDown={handleKeyDown} className="h-8 text-right" />
                ...
                <td className="p-2 align-middle w-[100px] text-right font-medium">
                    {isEditing ? (
                        <Input type="number" value={localItem.extended || ""} onChange={(e) => handleChange("extended", parseFloat(e.target.value))} onKeyDown={handleKeyDown} className="h-8 text-right" />
                    ) : (
                    ...
                    <div className="flex items-center gap-0.5 justify-end">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                <DropdownMenuItem onSelect={() => onInsert(item._id, "above")}>Insert Row Above</DropdownMenuItem>
                                <DropdownMenuItem onSelect={() => onInsert(item._id, "below")}>Insert Row Below</DropdownMenuItem>
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onSelect={() => { setAddingSubItem(true); setSubExpanded(true) }}>
                ...
                <tr className="bg-violet-500/5 border-b border-violet-500/20">
                    <td className="p-2 align-middle" colSpan={4}>
                        <div className="flex items-center gap-1 pl-6">
                            <span className="text-xs text-muted-foreground mr-1">Sub-item:</span>
                            <Input
                                placeholder="Spec No" value={newSub.spec_no}
                                onChange={(e) => setNewSub((s) => ({ ...s, spec_no: e.target.value }))}
                    ...
                    <td className="p-2 align-middle" colSpan={2}>
                        <div className="flex gap-1">
                            <Input placeholder="Qty" value={newSub.qty} onChange={(e) => setNewSub((s) => ({ ...s, qty: e.target.value }))} className="h-7 text-xs w-20" />
                            <Input placeholder="Unit $" type="number" value={newSub.unit_cost} onChange={(e) => setNewSub((s) => ({ ...s, unit_cost: e.target.value }))} className="h-7 text-xs w-24" />
                        </div>
                    </td>
                    <td className="p-2 align-middle" colSpan={4}>
                        <div className="flex items-center justify-end gap-1">
                            <Button size="sm" className="h-7 text-xs bg-violet-500 hover:bg-violet-600 text-white" onClick={handleAddSubItemSave}>Add</Button>
                            <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setAddingSubItem(false)}>Cancel</Button>
                        </div>
                    </td>
                </tr>
            )}
            ...
            <DeleteRowDialog
                ...
                onConfirm={() => { onDelete(item._id); setDeleteDialogOpen(false) }}

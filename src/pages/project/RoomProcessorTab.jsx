...
function Lightbox({ images, startIndex, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center pointer-events-auto">
      <button onClick={onClose} className="absolute top-4 right-4 h-10 w-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors">
      ...
      {hasPrev && (
        <button onClick={() => setIdx((i) => i - 1)} className="absolute left-4 h-10 w-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors">
          <ChevronLeft className="h-5 w-5 text-white" />
        </button>
      )}
      <div className="max-w-[90vw] max-h-[86vh] flex flex-col items-center gap-3">
        <img key={url} src={url} alt={img.name || img.filename} className="max-w-full max-h-[80vh] object-contain rounded-xl shadow-2xl" />
        <div className="flex items-center gap-3">
          <span className="text-white/50 text-xs font-mono bg-white/[0.08] rounded-full px-3 py-1">{img.name || img.filename}</span>
          <a href={url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-white/40 hover:text-white/70 text-xs transition-colors">
            <ExternalLink className="h-3 w-3" /> Open original
          </a>
        </div>
      </div>
      {hasNext && (
        <button onClick={() => setIdx((i) => i + 1)} className="absolute right-4 h-10 w-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors">
  ...
  return (
    ...
    >
      ...
      <div className="px-2.5 py-2 border-t flex flex-col justify-between gap-2 min-w-0 bg-card border-border/50 h-[80px]">
        <span className="flex-1 min-w-0 text-[12px] font-semibold font-mono truncate leading-tight text-foreground/80 whitespace-normal" title={room.name}>
          {room.name}
        </span>
        ...
        <Button
          size="sm"
          className={`w-full gap-2 text-xs h-7 ${state === "done"
...
export function RoomProcessorTab({ project }) {
  ...
  const openLightbox = (roomsArray, clickedRoomId) => {
    const idx = roomsArray.findIndex((r) => r.id === clickedRoomId || r.name === clickedRoomId);
  ...
  if (Object.keys(groupedRooms).length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-muted-foreground">
        ...
        <h3 className="text-lg font-semibold text-foreground mb-1">No Rooms Extracted yet</h3>
        <p className="text-sm">Use the Room Separator tab to cut and save rooms from your pages first.</p>
      </div>
    );
  }
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-background">
      ...
      <div className="shrink-0 bg-violet-500/5 border-b border-violet-500/15 px-6 py-3 flex items-center gap-3">
        ...
        <p className="text-xs text-muted-foreground">
          Click <strong>"Add to Budget"</strong> on each room to auto-create a budget line item.
          Then go to the <strong>Budget tab</strong> to fill in Spec No, Vendor, Qty, and Unit Cost.
        </p>
      </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {rooms.map((room) => (
                <RoomCard
                  ...
                  pageNum={pageNum}
                  projectId={projectId}
                  onDoubleClick={() => openLightbox(rooms, room.id || room.name)}
                />
              ))}
            </div>
          </div>
